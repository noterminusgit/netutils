...
- Outputs timestamped CSV files: `output/<timestamp>-endpoints.csv` and `output/<timestamp>-cdp_neighbors.csv`
...
7. Write results to `output/<timestamp>-endpoints.csv` and `output/<timestamp>-cdp_neighbors.csv`
8. If reconciling, write `output/<timestamp>-endpoints_still_connected.csv` with only the devices found in both scans
...
Reconcile with previous CSV (leave blank to skip): output/2026-01-14T08-00-00Z-endpoints.csv
Loaded 35 MAC(s) from previous scan: output/2026-01-14T08-00-00Z-endpoints.csv
...
Endpoints written to: output/2026-01-15T10-30-45Z-endpoints.csv
...
Reconciled output written to: output/2026-01-15T10-30-45Z-endpoints_still_connected.csv

CDP neighbors written to: output/2026-01-15T10-30-45Z-cdp_neighbors.csv
...
**Endpoints CSV (output/2026-01-15T10-30-45Z-endpoints.csv):**
```csv
...
**CDP Neighbors CSV (output/2026-01-15T10-30-45Z-cdp_neighbors.csv):**
```csv
Switch Hostname,Switch IP,Neighbor Device ID,Neighbor IP,Local Port,Description,Remote Port,Platform,Capabilities
SWITCH-01,192.168.1.10,SWITCH-02.domain.com,10.0.0.2,Gi1/0/25,UPLINK-SW02,Gi1/0/1,cisco WS-C3750-48P,Router Switch IGMP
SWITCH-01,192.168.1.10,CORE-SW.domain.com,10.0.0.1,Te1/1/1,UPLINK-CORE,Te1/0/1,cisco C9300-48P,Router Switch IGMP
SWITCH-02,192.168.1.11,SWITCH-01.domain.com,10.0.0.3,Gi1/0/25,UPLINK-SW01,Gi1/0/25,cisco C9200-48P,Router Switch IGMP
```
...
6. **CSV Output**: Writes endpoints to `output/<timestamp>-endpoints.csv` and CDP neighbors to `output/<timestamp>-cdp_neighbors.csv`
7. **Reconciliation** (optional): If a previous CSV was provided, filters current results to only endpoints whose MAC addresses appeared in the previous scan, and writes to `output/<timestamp>-endpoints_still_connected.csv`
...
- Run a full scan as normal and write `output/<timestamp>-endpoints.csv`
- Compare MAC addresses between the previous and current scan
- Write `output/<timestamp>-endpoints_still_connected.csv` containing only endpoints present in both scans (with current data)
